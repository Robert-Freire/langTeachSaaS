...
test('progressive streaming: vocabulary words appear during streaming via route intercept', async ({ browser }) => {
  test.setTimeout(TEST_TIMEOUT)
  const context = await createMockAuthContext(browser)
  const page = await context.newPage()
  ...
  try {
    ...
    await expect(page.locator('h1')).toHaveText('Lesson Details', { timeout: UI_TIMEOUT })
    const lessonTitle = `Progressive Stream Test ${Date.now()}`
    await page.getByTestId('input-title').fill(lessonTitle)
    ...
    await page.getByTestId('input-topic').fill('Greetings')
    ...
    // Persist section so sectionId exists
    const presentationSection = page.getByTestId('section-presentation')
    await presentationSection.fill('French greetings vocabulary.')
    await presentationSection.blur()
    await expect(page.getByTestId('saved-indicator')).toBeVisible({ timeout: FEEDBACK_TIMEOUT })

    // Intercept the vocabulary stream with a fully-formed SSE body
    await page.route('**/api/generate/vocabulary/stream', async (route) => {
      const sseBody = [
        `data: ${JSON.stringify(JSON.stringify({ items: [{ word: 'bonjour', definition: 'hello' }] }))}\n\n`,
        'data: [DONE]\n\n',
      ].join('')
    ...
    // Wait for streaming or done state — output should contain the word
    const output = page.getByTestId('generate-output')
    await expect(output).toContainText('bonjour', { timeout: AI_STREAM_TIMEOUT })

    // After done, "Generated preview" header should appear
    await expect(page.getByText('Generated preview')).toBeVisible({ timeout: AI_STREAM_TIMEOUT })
  } finally {
    await context.close()
  }
})
