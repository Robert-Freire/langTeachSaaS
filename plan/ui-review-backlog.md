...
| #1328 | 2026-05-23 | review-ui could not capture live screenshots of /groups/new and /groups/:id/edit (Docker e2e builds from main repo, worktree code not included -- pre-existing limitation #1113). DS violations fixed inline (chip bg, ghost Cancel). Empty-state icon uses Inter not Manrope headline -- pre-existing in groups-empty from #1327. Inline typeahead glassmorphism pattern not yet in DS; Vera to document. |
| #1351 | 2026-05-24 | "Por encima del nivel" annotation badge in TaggedSpan popover uses a rounded-full violet pill; DS does not specify a shape/token for level-filter status annotations inside popovers (CEFR chips are square rounded-md). Vera to specify the annotation-badge token. Cosmetic, not blocking. |
