...
| #983 | 2026-04-26 | Sophy: add a C# post-parse guard in `ReflectionExtractionService` to enforce the cross-field invariant in code (if topicTags non-empty and whatWasCovered is null, synthesise fallback or reject). Prompt-only enforcement is fragile under model drift. | Low |
