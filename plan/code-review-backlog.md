...
- *#1401 (arch, low): `MapSessionToUpdateRequest` and `TryDeserialize<T>` are now duplicated verbatim in both `GroupSessionLogsController.cs` and `SessionLogsController.cs`. Extract to a shared static helper when a third PATCH endpoint is added or a controller cleanup sprint occurs.*
...
- *#1414 (arch, convention, cosmetic): `PendingFollowups.tsx`'s general-note add row uses `data-testid="general-note-input"`/`general-note-add-btn`, diverging from the sibling `StudentFollowupsCard.tsx` pattern (`followup-input`/`followup-add-btn`) for a functionally identical add row. Intentional to avoid ambiguity with the per-row `followup-dot-{id}` testids already in the same file; unit tests and the visual spec are internally consistent with it. No action needed unless a shared add-input component is later extracted.*
