...
| #656 | 2026-04-11 | low | Telegram flow creates session log drafts from extracted reflections but never creates VoiceNote entities (audio arrives as text notes). VoiceNoteId is therefore always null for Telegram-originated logs. If Telegram voice messages should be tracked as VoiceNote entities in future, that requires a separate task to save the Telegram audio blob as a VoiceNote before extraction. |
