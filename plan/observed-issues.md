...
| #909 | 2026-04-24 | low | NextSessionHero.tsx: `lastSessionTopicTags.filter((t) => t.trim())` crashed when array contained null values (seed data). Fixed inline in this task (null-guard added). Root cause: backend may return null entries in array; worth adding API-level validation to prevent nulls in tag arrays. |
...
| #932 | 2026-04-24 | low | Sophy: PromptService.BuildSystemPrompt hardcodes instructional English text as C# string literals (native-language bullets, CEFR cue, personalization directives). Should live in a prompt-fragments config. Each prompt fix PR is a symptom; when the fix count hits 5, extract text into data. |
| #964 | 2026-04-26 | low | Students page header: "New student via voice" (variant="secondary") renders without a visible fill, appearing as near-ghost next to the filled gradient "Add Student" primary. DS §5 forbids ghost adjacent to filled primary. Pre-existing button styling, not introduced by #964; the secondary variant in the shadcn config does not give the button a tonal background. |
