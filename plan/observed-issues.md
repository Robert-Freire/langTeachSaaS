...
| #167 | 2026-03-25 | minor | GenerationWarnings is stored once at creation and never recomputed after AddEntry/UpdateEntry/DeleteEntry/Reorder. If a teacher manually edits flagged sessions, warning sessionIndex may reference the wrong entry. Deferred: clearing warnings on every edit would be confusing UX; teacher can dismiss individually. Revisit if a re-validate action is added. |
| #167 | 2026-03-25 | minor | DismissWarning is a read-modify-write on a JSON blob with no optimistic concurrency. Theoretical race if two dismissals fire simultaneously. Not a real risk in a single-teacher app (deliberate UI clicks); mirrors accepted pattern from PRs #64/#284. Revisit if multi-user editing is introduced. |
| #290 | 2026-03-26 | minor | StudentForm mobile (375px): 3 header action buttons (Create Course, Cancel, Update Student) overflow and overlap the page title. Pre-existing issue from #289 adding the 3rd button; not introduced by #290. |
