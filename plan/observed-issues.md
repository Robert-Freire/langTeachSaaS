...
| #593 | 2026-04-07 | P3:nice | Weakness-type allow-list hardcoded in 3 places (StudentService, PromptService, frontend TS union); should be a single source from pedagogy config — filed #595 |
...
| #586 | 2026-04-07 | low | WeaknessType allow-list ("grammatical"/"lexical"/"orthographic") duplicated in SanitizeWeaknesses and SectionProfileService; could drift if a 4th type is added. Consider a shared const. |
| #586 | 2026-04-07 | low | LessonPlanUserPrompt error profile numbering is global across type groups (1,2,3...) rather than restarting per-group. Cosmetic preference. |
| #586 | 2026-04-07 | low | StudentWeakness is defined in LangTeach.Api.AI namespace but used by CourseService and GenerateController (Services/Controllers layer). Consistent with existing GenerationContext/CurriculumContext pattern but architecturally a cross-layer type. Would need a shared DTO or a move to fix cleanly. |
