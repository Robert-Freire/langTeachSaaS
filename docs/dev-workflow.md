...
flowchart TD
    ...
    subgraph QUALITY ["Quality Gates"]
        ...
        R["architecture-reviewer agent\n+ conditional reviewers per review-routing.md"]
        R --> S{Review verdicts}
...
**Planning** happens inside the worktree. The agent writes a plan file, then runs the `review-plan` agent to validate the approach against the actual codebase (not assumptions). The reviewer also runs a **behavior change ripple check**: it greps for parallel implementations of any behavior the plan modifies (validation rules, labels for domain concepts, error message shapes, default values, permission checks, UI patterns for the same concept) and forces the plan to enumerate every parallel occurrence with an explicit disposition (update, leave as legacy, or file a follow-up issue). This is the highest-leverage check: many regressions come from changing behavior in screen A without updating screens B, C, D. When the reviewer flags issues, the agent does not stop. It critically evaluates each finding, fixes what is valid, pushes back with reasoning on what it disagrees with, and re-runs the review. Only if the agent and reviewer fundamentally disagree after two rounds does the user get involved. This keeps the human out of mechanical iteration while preserving their authority on real architectural disputes.
...
**Quality Gates** run after pre-push checks pass. The first gate (`qa-verify`) is sequential; the next batch (`architecture-reviewer`, and conditionally `prompt-health-reviewer`, `Sophy data model review`, and/or `pedagogy-reviewer`) run sequentially; the last (`review-ui`) is conditional. Line-level code review (bugs, style, unused imports) is handled by CodeRabbit post-push rather than a pre-push `review` agent.
...
- `architecture-reviewer` has a structural lens: it cross-references the diff against the rest of the codebase to detect inconsistent patterns, duplicated logic, missing reuse of shared utilities, and convention breaks. It reads 3-5 similar existing files for each changed file and compares. It also runs a **behavior parallel check** as a safety net for the ripple check: if the diff changes behavior X in file A and another file B (not in the diff) implements the same conceptual rule differently, it flags Inconsistency. This catches divergence that plan-review missed. The canonical failure case it prevents: a new CI workflow that runs `npm run build` without the VITE_* env vars that the existing `frontend.yml` passes to the same command (this was caught by CodeRabbit on PR #197 but missed by the code reviewer). Minor notes go to `plan/code-review-backlog.md`; NEEDS REVISION verdicts require fixes before proceeding.
...
1. **Backlog triage (PM, interactive):** The PM reads the three backlog files (`plan/code-review-backlog.md`, `plan/ui-review-backlog.md`, `plan/observed-issues.md`) and classifies each entry as FIX NOW (blocks sprint quality), NEXT SPRINT (batch into a themed issue), or DELETE (not worth tracking). The user reviews and approves the triage. FIX NOW items get implemented via normal worktree flow before proceeding. NEXT SPRINT items are grouped into `type:polish` or `type:tech-debt` issues that go through the standard `qa-ready` agent gate.

**Branch-level review (Stage 1b):** Before the sprint-close agent runs, four reviewers (`sophy`, `architecture-reviewer`, `prompt-health-reviewer`, `security-reviewer`) run sequentially against the full sprint diff. Each is prompted to focus on cross-cutting concerns and to perform a **mandatory consistency sweep**: scan for parallel implementations of the same behavior across tasks (validation rules, labels, error message formats, defaults, permission checks, empty/loading/error state conventions). When the same conceptual rule appears with two different implementations across tasks in the sprint, it gets flagged as Inconsistency. This is the highest-value sprint-level finding because it catches "screen A was polished but screen B still uses the old pattern" issues that per-PR review cannot see.
...
Requires: `pip install pyodbc`, `az login`, ODBC Driver 18, Docker SQL running (`docker compose up sqlserver`). Use `--dry-run` to preview without writing.
...
| Consistency review | `architecture-reviewer` agent | PASS or PASS WITH NOTES |
| Prompt health (per-PR) | `prompt-health-reviewer` agent | CLEAN or NEEDS CLEANUP (only if `PromptService.cs` changed); URGENT blocks push |
| Data model review (per-PR) | Sophy (via general-purpose agent) | APPROVE or NEEDS CLARIFICATION (only if diff touches Models, DTOs, Migrations, data JSON, contentTypes); NEEDS CLARIFICATION blocks push |
| Pedagogy config review (per-PR) | `pedagogy-reviewer` agent (Isaac) | SOUND, ADJUST, or RETHINK (only if diff touches `data/pedagogy/*.json`, `data/section-profiles/*.json`, `data/pedagogy/cefr-level-rules/*.json`); RETHINK blocks push |
| Line-level code review | CodeRabbit (post-push) | Findings addressed via follow-up commits on the PR |
...
|------|----------------|-------------|
| `plan/code-review-backlog.md` | Unfixed findings from `architecture-reviewer` and CodeRabbit | PM at sprint end |
