...
- `review` performs a code review against the sprint branch (auto-detected), looking for architectural issues, security concerns, and convention violations. Minor notes that are not worth fixing immediately are logged to `plan/code-review-backlog.md` rather than blocking the PR.
