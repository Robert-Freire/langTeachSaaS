...
function Editor({ parsedContent, rawContent, onChange, onRegenerate, isIncomplete }: EditorProps) {
  ...
  const { fillInBlank, multipleChoice, matching, sentenceOrdering = [] } = content
  ...
  // Fragments stored as string[] but edited as a comma-separated string in the UI
  const updateSoFragments = (i: number, value: string) => {
    const fragments = value.split(',').map(s => s.trim())
    // Recompute correctOrder to be identity (0,1,2,...) when fragments change length
  ...
  const updateSoCorrectOrder = (i: number, value: string) => {
    // User enters comma-separated 1-based indices; convert to 0-based
...
function Preview({ parsedContent }: PreviewProps) {
  ...
  const { fillInBlank, multipleChoice, matching, sentenceOrdering = [] } = parsedContent as ExercisesContent
...
function Student({ parsedContent, rawContent }: StudentProps) {
  ...
  const { fillInBlank, multipleChoice, matching, sentenceOrdering = [] } = validContent
  ...
  const totalQuestions = fillInBlank.length + multipleChoice.length + matching.length + sentenceOrdering.length
  const totalCorrect = [
    ...fibCorrect,
    ...mcCorrect,
    ...matchCorrect,
