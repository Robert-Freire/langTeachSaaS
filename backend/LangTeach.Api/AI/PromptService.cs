...
public class PromptService : IPromptService
{
    ...
    public ClaudeRequest BuildReflectionExtractionPrompt(ReflectionExtractionContext ctx)
    {
        ...
        var system = $"""
            ...
            - newSessionTitle: string or null — concise title (under 60 chars) for a NEW session record the teacher is creating, either scheduled for the future or retroactively registered for a past date. Set for explicit scheduling statements ("next Monday I want to do a session on the subjunctive", "la semana que viene hagamos una sesión sobre el subjuntivo"), for retrospective registration when the teacher explicitly says they forgot to register a past session ("que se me olvidó registrarla", "apunta una sesión del lunes pasado sobre X"), and for any past-tense session description when no open session is in scope (see IMPORTANT CONTEXT above if present). Use sessionTitle instead when an open session is in scope and the teacher is describing that session. Distinct from nextLessonIdeas (planning ideas without a scheduled appointment). Both newSessionTitle and nextLessonIdeas may be set simultaneously if the teacher is scheduling AND has broader ideas.
