...
public class PromptService : IPromptService
{
    ...
    public ClaudeRequest BuildReflectionExtractionPrompt(ReflectionExtractionContext ctx)
    {
        ...
        var sessionContextHint = ctx.HasOpenSession
            ...
            Exception — present-day anchor: if the teacher's statement opens with a present-day anchor ("hoy", "en la clase de hoy", "esta mañana", "este mediodía", "today", "this morning", or an explicit time for today such as "de las 10:00"), treat this as a live post-class reflection for TODAY's session. Set sessionDate=today, populate sessionTitle and whatWasCovered as normal, and leave newSessionTitle and newSessionDate null. Past-tense verbs ("hemos trabajado", "hicimos") in a reflection that opens with a present-day anchor do NOT indicate retrospective registration.
            Otherwise: if the teacher describes a class that already happened without a present-day anchor (cues: "le di clase", "ayer trabajamos", "la semana pasada hicimos"), treat it as retrospective session registration: populate newSessionTitle and newSessionDate; sessionTitle and whatWasCovered do not apply.
            """;
        ...
        var system = $"""
            ...
            - sessionDate: string or null — ISO 8601 date (YYYY-MM-DD) of the session being described. Resolve date references using today's date and day of week: "hoy"/"today" = today, "ayer"/"yesterday" = yesterday, {weekdayBackwardRule} Null if no date is mentioned. The opening clause of the teacher's statement is the primary temporal anchor: if the opening contains "hoy", "en la clase de hoy", or a specific time for today, sessionDate=today, even if past dates appear later in the body (e.g. "hemos repasado la pizarra del 30 de abril" is a content reference, not a session date anchor).
