...
public class PromptService : IPromptService
{
    ...
    public ClaudeRequest BuildReflectionExtractionPrompt(ReflectionExtractionContext ctx)
    {
        ...
        var system = $"""
            ...
            - newSessionTitle: string or null — concise title (under 60 chars) for a NEW future session the teacher is scheduling. Set only for forward-looking scheduling statements where the teacher explicitly proposes a future session with a topic (e.g. "next Monday I want to do a session on the subjunctive", "la semana que viene hagamos una sesión sobre el subjuntivo"). Null when the teacher is describing the current or past session. Distinct from sessionTitle (the past session) and from nextLessonIdeas (which captures planning ideas without a scheduled appointment). Both newSessionTitle and nextLessonIdeas may be set simultaneously if the teacher is scheduling AND has broader ideas.
            - newSessionDate: string or null — ISO 8601 date (YYYY-MM-DD) for the proposed new session. Resolve relative dates FORWARD from today: "next Monday" = the next Monday strictly after today, "la semana que viene" = same day next week. Null if no specific day or date can be inferred (e.g. "next class", "soon"). Null if newSessionTitle is null. If a date is mentioned without a topic, set both fields to null.
    ...
    public ClaudeRequest BuildStudentProfileExtractionPrompt(string text)
    {
        ...
        var system = $"""
            ...
            - skillLevel: object or null — per-skill CEFR level overrides when the teacher explicitly states a different level for a specific skill. Keys: "reading", "writing", "speaking", "listening". Values are CEFR level strings (e.g. "B1", "C1", "B1.2"). Omit a key if not explicitly mentioned. Null if no per-skill levels mentioned at all. Do NOT emit skillLevel entries when the teacher says not to change the level ("no lo toco", "no cambies el nivel").
