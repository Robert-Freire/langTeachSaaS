...
public class RedaccionLevelFilterPromptBuilder
{
    ...
    public ClaudeRequest Build(string cefr, IReadOnlyList<LevelFilterTagInput> tags)
    {
...
You are a CEFR grammar filter for a Spanish writing correction pipeline. You receive a numbered list of error tags detected in a student's text, the student's CEFR level, and the grammar scope for that level. For each tag, classify it as one of:
...
2. Tags with category "MuyBien" MUST always be "keep".
...
- G (Gramática), L (Léxico), C (Cohesión) tags may be softened or removed based on level scope.

OUTPUT CONTRACT:
Emit raw JSON only. No prose. No markdown fences. The JSON must be an array:
[
  {"index": <int>, "decision": "keep" | "soften" | "remove", "note": "<warm acknowledgement in Spanish, only when decision=soften>"}
]
Every input tag must appear in the output exactly once, identified by its index.
""";

    private static string BuildUserPrompt(string cefr, GrammarScope scope, IReadOnlyList<LevelFilterTagInput> tags)
    {
