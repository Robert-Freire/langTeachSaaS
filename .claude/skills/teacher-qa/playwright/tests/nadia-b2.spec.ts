/**
 * Nadia Persona — B2 Spanish teacher, Arabic L1 student
 *
 * Teacher: Nadia. Student: [QA] Youssef. Level: B2.1. L1: Arabic.
 * Template: Conversation. Topic: "debating urban development and heritage conservation"
 *
 ...
 *   npx playwright test tests/nadia-b2.spec.ts --config playwright.config.ts
 */
...
const PERSONA = {
  ...
  student: {
    ...
    difficulties: [
      { description: 'article gender agreement', category: 'Grammar', severity: 'Medium' },
      { description: 'written accent marks (tildes)', category: 'Writing', severity: 'Medium' },
    ],
  },
  lesson: {
    templateName: 'Conversation',
    title: '[QA] Urban Development and Heritage Conservation — B2.1',
    language: 'Spanish',
    cefrLevel: 'B2',
    topic: 'debating urban development and heritage conservation',
  },
}

test('Nadia B2.1 — create student, generate lesson, capture output', async ({ browser }) => {
  const outputDir = path.resolve(
    __dirname,
    '../../output',
    `nadia-b2-${new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19)}`
  )

  const context = await createQAAuthContext(browser)
  const page = await context.newPage()

  // 1. Ensure student [QA] Youssef exists (create if first run, reuse on subsequent runs)
  ...
  console.log(`Nadia B2.1 run complete. Output: ${outputDir}`)
