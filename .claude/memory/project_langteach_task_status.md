...
- Student-Aware Curriculum: ACTIVE (#206, #256, #257, #258, #259, #260, #261, #262, #253, #254, #152, #167, #166, #267, #268, #289, #290, #291, #292)
  - #290 DONE — PR #295 merged (StudentCoursesCard on student edit page with student-specific empty state; CourseDetail error state with back nav + retry; data-testid on generation error card; 2 new e2e tests: full happy path from student edit + generation failure; 5 unit tests)
  - #289 DONE — PR #293 merged ("Create Course" button on student edit page; dedicated getStudent query for locked student in CourseNew; pre-fills language+CEFR; tooltip on disabled; 6 unit + 1 e2e tests)
  - #167 DONE — PR #286 merged (AI generation quality guardrails: post-generation CEFR validation via Claude Sonnet; CurriculumValidationService; GenerationWarnings + DismissedWarnings JSON columns; warnings panel in CourseDetail with per-session dismiss; 7 validation + 4 controller + 4 frontend unit tests; e2e spec)
  - #259 DONE — PR #284 merged (add/remove curriculum sessions; drag-to-reorder via @dnd-kit; curriculum summary header stat chips; soft-delete+reindex on CurriculumEntry; IsDeleted filter across all entry queries; 9 backend + 9 frontend unit tests; e2e tests updated)
  ...
  - #268 DONE — PR #279 merged (CefrProductionGuidance helper A1/A2/B1/B2/C1/C2; Production MANDATORY in every lesson plan; practice ordering controlled-to-meaningful; backend warning for <5 sections; 9 unit tests; Teacher QA run pending)
  - #258 DONE — PR #281 merged (VocabularyThemes field added to CurriculumEntry; CourseDetail expandable cards: collapsed=topic+grammar+status, expanded=skills+vocab+context+rationale; mobile-responsive 44px tap targets; 5 new unit tests; e2e expand toggle test)
  - #262 DONE — PR #282 merged (SessionMappingService: expand/compress/exact strategies; GET {level}/mapping?sessionCount=N endpoint; SessionMappingPreview React component; CourseNew always shows session count selector; curriculum entries generated from mapping not raw units; 14 backend + 9 frontend unit tests; 2 e2e tests)
  - #260 DONE — PR #280 merged (CurriculumObjectives field in GenerationContext; PEDAGOGICAL CONSTRAINTS section in LessonPlanUserPrompt; lesson.Objectives flows into AI prompt via GenerateController; improved objectives string construction; section count warning + ValueKind guard; 5 new tests)
